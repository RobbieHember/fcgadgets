...
def Process1_ImportVariables(meta,pNam):
	...
	if meta[pNam]['Project']['Code Project']=='BCFCS_LUC':
		...
		mask='BCFCS_EvalAtCN'
	elif meta[pNam]['Project']['Code Project']=='BCFCS_NOSE':
		mask='BCFCS_NOSE'
	elif meta[pNam]['Project']['Code Project']=='BCFCS_NMC':
		mask='BCFCS_NMC'
	elif meta[pNam]['Project']['Code Project']=='BCFCS_FNM_HarvRest':
		mask='BCFCS_NMC'
	...
	if 'Custom SI Source' in meta[pNam]['Project']:
		...
		if pNam=='BCFCS_NMC':
			...
			for iU in range(u.size):
				...
				lsat['SI'][ind]=meta['Param']['BE']['ByBGCZ'][cd]['SI Calibration for FNM']
		elif pNam=='BCFCS_NOSE':
			lsat['SI']=18*np.ones(meta[pNam]['Project']['N Stand'])
			u=np.unique(lsat['ID_BGCZ'])
			for iU in range(u.size):
				cd=cbu.lut_n2s(meta['LUT']['BEC_BIOGEOCLIMATIC_POLY']['ZONE'],u[iU])[0]
				ind=np.where(lsat['ID_BGCZ']==u[iU])[0]
				lsat['SI'][ind]=meta['Param']['BE']['ByBGCZ'][cd]['SI Calibration for NOSE']
		else:
			lsat['SI']=18*np.ones(meta[pNam]['Project']['N Stand'])
			u=np.unique(lsat['ID_BGCZ'])
			for iU in range(u.size):
	...
	# Wood density by species (kg/m3)
	lsat['Wood Density']=meta['Param']['BE']['Biophysical']['Density Wood Standard']*np.ones(meta[pNam]['Project']['N Stand'],dtype='int16')
...
# 			lsat['OAF1'][ind0]=100*meta['Param']['Raw']['TreeDensityClassStatistics']['OAF1'][ind1[0]]
	...
	for iStand in range(meta[pNam]['Project']['N Stand']):
		...
		if meta[pNam]['Project']['Special Attribution Method']=='NOSE':
			dmec0[iStand]['Index to Event Inciting NOSE']=9999*np.ones(N_Events_Init,dtype='int16')
	...
	if meta[pNam]['Project']['DMEC Method']=='From Events':
		...
		# Add aerial nutrient application observations
...
def Define_NOSE_ProjectType(meta,pNam,dmec0,lsat):
	...
	meta[pNam]['Project']['ASET']=meta['LUT']['Derived']['ASET']['Unknown']*np.ones(meta[pNam]['Project']['N Stand'],dtype='int16')
	meta[pNam]['Project']['Strata']['Project Type']['ID']=meta['LUT']['Derived']['ASET']['Unknown']*np.ones(meta[pNam]['Project']['N Stand'],dtype='int16')
		...
		if dmec0[iStand]['ASET'][iNOSE]==meta['LUT']['Derived']['ASET']['Back-to-back Planting']:
			...
			if iIncite.size==0:
				...
				if iIncite.size>0:
					...
					dmec0[iStand]['Mortality Factor'][iIncite]=100
					dmec0[iStand]['Index to Event Inciting NOSE'][iNOSE]=0
			...
			# Give first event as dummy answer and then troubleshoot to see if it can be classified as something
			#dmec0[iStand]['Index to Event Inciting NOSE'][iNOSE]=0
				...
				for iT in range(dmec[iScn][iStand]['Year'].size):
					if np.isin(iScn,meta[pNam]['Project']['Baseline Indices'])==True:
						...
						try:
							...
							print(iT)
							print(iNOSE.size)
							print(iIncitingEvent.size)
							print(dmec[iScn][iStand]['ASET'][iT])
							#print(dmec[iScn][iStand])
							#print(meta[pNam]['Project']['ASET'][iStand])
							return
...
def PrepareInventoryFromSpreadsheet(meta,pNam):
	dGY=gu.ReadExcel(meta['Paths'][pNam]['Data'] + '\\Inputs\\GrowthCurvesTIPSY_Parameters.xlsx',sheet_name='Sheet1',skiprows=6)
	...
	for iScn in range(0,meta[pNam]['Project']['N Scenario']):
		for iBat in range(0,meta[pNam]['Project']['N Batch']):
			...
			# Save
			gu.opickle(meta['Paths'][pNam]['Input Scenario'][iScn] + '\\Inventory_Bat' + cbu.FixFileNum(iBat) + '.pkl',lsat)
...
def BuildEventChronologyFromSpreadsheet(meta,pNam):
	for iScn in range(meta[pNam]['Project']['N Scenario']):
		for iEns in range(meta[pNam]['Project']['N Ensemble']):
			for iBat in range(meta[pNam]['Project']['N Batch']):
				indBat=cbu.IndexToBatch(meta[pNam],iBat) # Index to batch
				tv=np.arange(meta[pNam]['Project']['Year Start'],meta[pNam]['Project']['Year End']+1,1)
	...
	for iScn in range(meta[pNam]['Project']['N Scenario']):
		...
		for iBat in range(meta[pNam]['Project']['N Batch']):
			...
			for iGC in range(3):
				...
				gu.opickle(meta['Paths'][pNam]['Data'] + '\\Inputs\\Scenario' + cbu.FixFileNum(iScn) + '\\GrowthCurve' + str(iGC+1) + '_Bat' + cbu.FixFileNum(iBat) + '.pkl',G)
