...
#%%
def DeriveLandCoverComp1_v1(meta):
	...
	zE=u1ha.Import_Raster(meta,[],['elev'],'Extract Grid')
...
#%% Primary forest mask and validation based on Hill 2022
def ValidatePrimaryForestMask(meta):
	# Run province-wide ROI map first

	# Import raster data
	zRef=gis.OpenGeoTiff(meta['Paths']['bc1ha Ref Grid'])
	vList=['lc_comp1_2019','harv_yr_comp2','road_ften_s','road_atl']
	z0=u1ha.Import_Raster(meta,[],vList,'Extract Grid')
	
	# Import Hill (2022) map
	hill=gpd.read_file(r'C:\Data\Primary Forest\Hill 2022\HILL_Version_2022.shp').to_crs(meta['Geos']['crs'])
	hill=gpd.overlay(hill,roi['gdf']['bound ROI'],how='intersection')
	
	# Rasterize Hill
	shapes=((geom,value) for geom, value in zip(hill['geometry'],hill['Human_Impa']))
	zHill=5*np.ones(zRef['Data'].shape,dtype=float)
	burned=features.rasterize(shapes=shapes,fill=5,out=zHill,transform=zRef['Transform'])
	
	shapes=((geom,value) for geom, value in zip(hill['geometry'],hill['nfi_plot']))
	zNFI=np.zeros(zRef['Data'].shape,dtype=float)
	burned=features.rasterize(shapes=shapes,fill=0,out=zNFI,transform=zRef['Transform'])
	#u=gu.CountByCategories(zHill,'Percent')
	
	# Create estiamte of primary forests from consideration of harvesting and roads
	z1=copy.deepcopy(zRef)
	z1['Data']=4*np.ones(zRef['Data'].shape,dtype='int8')
	ind=np.where( (zRef['Data']==1) ); z1['Data'][ind]=3
	ind=np.where( (z0['lc_comp1_2019']==meta['LUT']['Derived']['lc_comp1']['Forest']) ); z1['Data'][ind]=2
	ind=np.where( (z0['lc_comp1_2019']==meta['LUT']['Derived']['lc_comp1']['Forest']) & (z0['harv_yr_comp2']>0) ); z1['Data'][ind]=1
	ind=np.where( (z0['lc_comp1_2019']==meta['LUT']['Derived']['lc_comp1']['Forest']) & (z0['road_ften_s']>0) ); z1['Data'][ind]=1
	ind=np.where( (z0['lc_comp1_2019']==meta['LUT']['Derived']['lc_comp1']['Forest']) & (z0['road_atl']>0) ); z1['Data'][ind]=1
	#plt.close('all'); plt.matshow(z1['Data'])
	
	#ind0=np.where( (z0['lc_comp1_2019']==meta['LUT']['Derived']['lc_comp1']['Forest']) & (zHill!=5) )
	ind0=np.where( (z1['Data']==3) & (zHill!=5) )
	indA=np.where( (z1['Data']==3) & (zHill==0) )
	indD=np.where( (z1['Data']==3) & (zHill!=0) & (zHill!=5) )
	uA=np.unique(zNFI[indA])
	uD=np.unique(zNFI[indD])
	fDisagree=indA[0].size/ind0[0].size
	fAgree=indD[0].size/ind0[0].size
	#xA=zRef['X'][indA];yA=zRef['Y'][indA]
	#xD=zRef['X'][indD];yD=zRef['Y'][indD]
	#idxA=gu.IndicesFromUniqueArrayValues(zNFI[indA])
	#idxD=gu.IndicesFromUniqueArrayValues(zNFI[indD])
	
	# Plot
	lab=['Disturbed forests','Primary forest','Non-forest land']
	N_vis=len(lab)
	N_hidden=1
	...
	meta['Graphics']['Map']['Legend X']=0.78
	
	cm=np.vstack( ((0,0,0,1),(0.5,0.5,0.5,1),(0.9,0.9,0.9,1),(1,1,1,1)) )
	...
	im=ax[0].matshow(z1['Data'],extent=roi['grd']['Extent'],cmap=cm)
	...
	#ax[1].set(position=[0.01,0.01,0.02,0.02])
	
	indA=np.where(np.isin(hill['nfi_plot'],uA)==True)[0]
	indD=np.where(np.isin(hill['nfi_plot'],uD)==True)[0]
	hill.plot(ax=ax[0],edgecolor=[0,0,0],facecolor='none',linewidth=0.5)
	hill.iloc[indA].plot(ax=ax[0],edgecolor=[0,1,0],facecolor='none',linewidth=1)
	hill.iloc[indD].plot(ax=ax[0],edgecolor=[1,0,0],facecolor='none',linewidth=1)
	
	zmn=np.min(z1['Data']); zmx=np.max(z1['Data']); cb_ivl=(zmx-zmn)/N_tot; cb_bnd=np.arange(zmn,zmx+cb_ivl-N_hidden*cb_ivl,cb_ivl)
	cb_ticks=np.arange(zmn+cb_ivl/2,N_tot-1,cb_ivl)
	cb=plt.colorbar(im,cax=ax[1],cmap=cm,boundaries=cb_bnd,ticks=cb_ticks)
	#ax[1].set(position=[0.78,0.75,0.03,0.1])
	...
	if meta['Graphics']['Print Figures']=='On':
		gu.PrintFig(meta['Graphics']['Print Figure Path'] + '\\' + roi['Name'] + '_PrimaryForestValidation','png',900)
	return

#%% Global Forest Change Loss Year (adjusted to remove known disturbances)
def FilterGFC_LossYear(meta):
	zRef=gis.OpenGeoTiff(meta['Paths']['bc1ha Ref Grid'])
	vList=['gfcly','harv_yr_comp1','fire_yr','ibm_yr']
	z=Import_Raster(meta,[],vList)
	ind=np.where( (np.abs(z['gfcly']['Data']-z['harv_yr_comp1']['Data'])<3) | (np.abs(z['gfcly']['Data']-z['fire_yr']['Data'])<2) | (np.abs(z['gfcly']['Data']-z['ibm_yr']['Data'])<2) )
	z['gfcly']['Data'][ind]=0
...
#%%
def CompareForestArea(meta):

	meta=u1ha.Init()
	zRef=gis.OpenGeoTiff(meta['Paths']['bc1ha Ref Grid'])

	lut_comp1=meta['LUT']['Derived']['lc_comp1']
	lut_lc_vri_l3=meta['LUT']['VEG_COMP_LYR_R1_POLY']['BCLCS_LEVEL_2']
	lut_lc_vri_l4=meta['LUT']['VEG_COMP_LYR_R1_POLY']['BCLCS_LEVEL_4']
	# meta['LUT']['VEG_COMP_LYR_R1_POLY']['BCLCS_LEVEL_5']

	# Import data
	z=u1ha.Import_Raster(meta,[],['lc_comp1_2019','lc_vri_l2','lc_ntems_2019_recl','lc_vri_recl','lc_cec_2010','lc_cec_2020','forfrac2m_Tom23','forfrac5m_Tom23', \
							   'gfc_tc2000','gfc_loss','gfc_gain'],'Extract Grid') # ,''fire_yl','harv_yr_comp1'

	# Fix global forest change
	z['gfc_tc2000']=z['gfc_tc2000']*zRef['Data']
	z['gfc_loss']=z['gfc_loss']*zRef['Data']
	z['gfc_gain']=z['gfc_gain']*zRef['Data']
	z['gfc_tc2023']=z['gfc_tc2000'].copy()
	z['gfc_tc2023'][ z['gfc_loss']>0 ]=0
	z['gfc_tc2023'][ z['gfc_gain']>0 ]=50 # Not sure what tree cover to assume

	A={}
	A['lc_comp1_2019']=np.where(z['lc_comp1_2019']==lut_comp1['Forest'])[0].size/1e6
	A['lc_vri_recl']=np.where(z['lc_vri_recl']==lut_comp1['Forest'])[0].size/1e6
	A['lc_ntems_2019_recl']=np.where(z['lc_ntems_2019_recl']==lut_comp1['Forest'])[0].size/1e6
	A['lc_cec_2010']=np.where(z['lc_cec_2010']==lut_comp1['Forest'])[0].size/1e6
	A['lc_cec_2020']=np.where(z['lc_cec_2020']==lut_comp1['Forest'])[0].size/1e6
	A['forfrac2m_Tom23']=np.sum(z['forfrac2m_Tom23'].astype('float')/100)/1e6
	A['forfrac5m_Tom23']=np.sum(z['forfrac5m_Tom23'].astype('float')/100)/1e6
	A['gfc_tc2000']=np.sum(z['gfc_tc2000'].astype('float')/100)/1e6
	A['gfc_tc2023']=np.sum(z['gfc_tc2023'].astype('float')/100)/1e6
	A

	luc={}
	luc['gfc loss']=np.where(z['gfc_loss']>0)[0].size/1e6
	luc['gfc gain']=np.where(z['gfc_gain']>0)[0].size/1e6
	luc['gfc net']=A['gfc_tc2023']-A['gfc_tc2000']
	luc

	luc_ann={}
	luc_ann['gfc loss']=np.where(z['gfc_loss']>0)[0].size/1e6/(2023-2000)
	luc_ann['gfc gain']=np.where(z['gfc_gain']>0)[0].size/1e6/(2023-2000)
	luc_ann['gfc net']=(A['gfc_tc2023']-A['gfc_tc2000'])/(2023-2000)
	luc_ann['cec loss']=np.where( (z['lc_cec_2010']==lut_comp1['Forest']) & (z['lc_cec_2020']!=lut_comp1['Forest']) )[0].size/1e6/(2020-2010)
	luc_ann['cec gain']=np.where( (z['lc_cec_2010']!=lut_comp1['Forest']) & (z['lc_cec_2020']==lut_comp1['Forest']) )[0].size/1e6/(2020-2010)
	luc_ann['cec net']=(A['lc_cec_2020']-A['lc_cec_2010'])/(2020-2010)
	luc_ann

	return

#%%
def Download_GFC(meta):
	ee.Authenticate(force=True)
	ee.Initialize(project='cdf-zone1')
	
	layL=['loss','gain','lossyear'] # 'treecover2000'
	for lay in layL:
		y0=ee.Image("UMD/hansen/global_forest_change_2023_v1_11").select(lay)
		
		# Clip
		geomPoly=ee.Geometry.BBox(-139,47.25,-113,60.1) # Province wide
		y1=y0.clip(geomPoly)
		
		# Reproject
		crs='EPSG:3005'
		scale=100
		y2=y1.reproject(crs=crs,scale=scale)
		
		# Define task
		task=ee.batch.Export.image.toDrive(image=y2,description='gfc_' + lay,scale=scale,maxPixels=1e13,fileFormat="GeoTIFF")
		task.start()

		# Fix extent
		#lay='lossyear'
		fin=r'C:\Data\Global Forest Change\gfc_' + lay + '.tif'
		fout=r'C:\Data\Global Forest Change\bc1ha_gfc_' + lay + '.tif'
		gis.ReprojectRasterAndClipToRaster(fin,fout,meta['Paths']['bc1ha Ref Grid'],meta['Geos']['crs'])
