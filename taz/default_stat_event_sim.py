...
def PredictWildfire_OnTheFly(meta,pNam,vi,iT,iScn,iEns,flag_sim_event):
	...
	# Hardwired
	if 'Hardwire_Wildfire_Po_Future' in meta[pNam]['Scenario'][iScn].keys():
		if vi['tv'][iT]>=meta[pNam]['Project']['Year Project']:
			Po[:]=meta[pNam]['Scenario'][iScn]['Hardwire_Wildfire_Po_Future']
...
#%%
def PredictMPB_OnTheFly(meta,pNam,vi,iT,iScn,iEns,flag_sim_event):
...
#%%
def PredictHarvest_OnTheFly(meta,pNam,vi,iT,iScn,iEns,V_Merch,Period):
...
# 				Po_H_Sat=0.01*np.maximum(0,Po_Sat+Rate*np.maximum(1,vi['tv'][iT]-2021))
	...
	if 'HarvestRestrictTSNA' in meta[pNam]['Scenario'][iScn]:
		if meta[pNam]['Scenario'][iScn]['HarvestRestrictTSNA']>0:
			if Period=='Future':
				ind=np.where( (meta['Modules']['NutrientApplication']['ResponseCounterHarvestRestriction']!=0) )[0]
				flag_NA[ind]=0
	...
	if indS.size>0:
		for i in range(indS.size):
			...
			if iAvailable.size>0:
				...
				if 'Status Future Pile Burns' in meta[pNam]['Scenario'][iScn]:
					if meta[pNam]['Scenario'][iScn]['Status Future Pile Burns']=='Off':
						iE=iAvailable[0]+1
					...
					elif meta[pNam]['Scenario'][iScn]['Status Future Pile Burns']=='On':
