...
if style=='Presentation':
    plt.rcParams.update(gu.Import_GraphicsParameters('Presentation'))
    cla=[0,0,0]
    clt=[0.7,0.7,0.7] # text
    cl1=[0.27,0.49,0.79]; 
    cl2=[0.6,0.9,0]; 
    cl3=[0.7,0.4,0.95]; 
    lw=1.25; 
    ...
    plt.rcParams.update(gu.Import_GraphicsParameters('spyder_fs7'))
    cla=[0.4,0.4,0.4]
    clt=cla
    cl1=[0.27,0.49,0.79]; 
    cl2=[0.6,0.9,0]; 
...
def Plot_THLB(meta,tv,AEF,thlb,iScn):
    ...
    ax.plot(meta['Year'],AEF*np.sum(thlb[iScn]['Baseline WithDef'],axis=1)[0]*np.ones(meta['Year'].size)/1e6,'k-',lw=lw,color=cl1,label='Baseline')
    ax.plot(meta['Year'],AEF*np.sum(thlb[iScn]['Actual WithDef'],axis=1)/1e6,'k--',color=cl2,lw=lw,label='Actual')
    ax.set(xlim=[1980,2040],xticks=np.arange(1800,2200,10),ylim=[0,24],yticks=np.arange(0,26,2),ylabel='Timber harvesting landbase (million ha)',xlabel='Time, years')
    ax.legend(loc='lower left',facecolor=[1,1,1],frameon=False)
    ax.yaxis.set_ticks_position('both'); ax.xaxis.set_ticks_position('both'); ax.tick_params(length=tickl)
...
def PlotCarbonPoolBarChart(meta,tv,mos,AEF,iScn,iPS,iSS):
    ...
    plt.close('all'); fig,ax=plt.subplots(1,2,figsize=gu.cm2inch(14,6)); bw=0.25; bw2=0.3; cl=np.array([[0.65,0.75,0.75],[0.7,0.8,0.9],[0.8,0.9,1]]).T; lab=['1820','1990','2100']
    ...
    ax[0].errorbar(x,y,yerr=[y-yl,yh-y],color='k',ls='',capsize=2)
    ...
    ax[0].set(position=[0.07,0.125,0.7,0.86],xlim=[0.5,6.5],xticks=np.arange(1,10,1), \
      xticklabels=['Biomass','Dead \nwood','Litter','Soil','In-use\nproducts','Dumps & \nlandfills'],ylim=[0,11],yticks=np.arange(0,120,2),ylabel='Carbon stock (GtC)',xlabel='')
    ...
    ax[1].errorbar(x,y,yerr=[y-yl,yh-y],color='k',ls='',capsize=2)
    ax[1].set(position=[0.84,0.125,0.14,0.86],xlim=[0.5,1.5],xticks=np.arange(1,2,1),xticklabels=['Total'],ylim=[0,26],yticks=np.arange(0,120,2),ylabel='Carbon stock (GtC)',xlabel='')
...
def PlotFluxesBarChart(meta,tv,mos,AEF,iScn,iPS,iSS):    
    ...
    ax.errorbar(np.arange(1,y.size+1,1)-bw2,y,yerr=[y-yl,yh-y],color='k',ls='',capsize=2)
    ...
    ax.errorbar(np.arange(1,y.size+1,1),y,yerr=[y-yl,yh-y],color='k',ls='',capsize=2)
...
def PlotForcingBarChart(meta,tv,mos,AEF,cmp,iPS,iSS):
    ...
    plt.close('all'); fig,ax=plt.subplots(1,figsize=gu.cm2inch(18,8.5)); bw=0.2; bw2=0.25; 
    ax.plot([0,nam.size+1],[0,0],'k-',color=[0.8,0.8,0.8])
    ...
    ax.errorbar(np.arange(1,ymu.size+1,1)-bw2,ymu,yerr=[ymu-yl,yh-ymu],color='k',ls='',capsize=2)
    ...
    ax.errorbar(np.arange(1,ymu.size+1,1),ymu,yerr=[ymu-yl,yh-ymu],color='k',ls='',capsize=2)
    ...
    ax.set(position=[0.07,0.14,0.92,0.82],xlim=[0.5,nam.size+.5],xticks=np.arange(1,nam.size+1,1),xticklabels=xlab, \
           ylim=[-100,100],yticks=np.arange(-100,200,10),ylabel='Forcing (MtCO$_2$e yr$^-$$^1$)')
...
#%% Harvest removals
def PlotHarvestRemovals(meta,mos,tv,AEF,iScn,iT,iPS,iSS,flg_ann,flg_dead,flg_hbs):
    ...
    H_HBS=gu.ipickle(r'C:\Users\rhember\Documents\Data\Harvest\HBS\AnnualSum.pkl')
    ...
    if flg_ann==1:
        ax.plot(tv[iT],y_Tot[iT],'-',color=[0.8,0.8,0.8],lw=0.5,label='Tot. merch. vol. (FCAST)')
    ax.plot(tv[iT],y_Tot_ma,'-',color='k',lw=1,label='Tot. merch vol. mov. ave. (FCAST)')
    
    if flg_dead==1:
        ax.plot(tv[iT],y_Dead_ma,'--',color='k',lw=1,label='Dead merch. vol. (FCAST)')
    
    if flg_hbs==1:
        ax.plot(H_FLNR['Year'],H_FLNR['Total_harvest_millions_m3'],'k-',color=cl1,lw=1.25,label='Tot. merch. vol. (FLNR)')
        ax.plot(H_HBS['Year'],H_HBS['V All m3']/1e6,'-c')
    
    #ax.plot(tv[iT],y,'-',color=cl2,lw=0.5,label='Total')
    ...
    if flag_printfigs=='On':
        nam_ps=meta['Project']['Strata']['Project']['Unique CD'][iPS]
        nam_ss=meta['Project']['Strata']['Spatial']['Unique CD'][iSS]
        gu.PrintFig(meta['Paths']['Figures'] + '\\' + nam_ps + '_' + nam_ss + '_HarvestVolumeMerch_Scn' + str(iScn+1),'png',900)
...
def PlotHarvestMultipleScenarios(meta,mos,tv,AEF,iT,iPS,iSS,iScn1,iScn2,flg_hbs):
    ...
    ax.plot(tv[iT],y,'-',color=cl1,lw=lw,label='Baseline scenario')
    
    y=AEF*gu.movingave(mos['Scenarios'][iScn2]['Sum']['V_ToMillMerchTotal']['Ensemble Mean'][iT,iPS,iSS],10,'historical')/1e6
    ax.plot(tv[iT],y,'--',color=cl2,lw=lw,label='Actual scenario')
...
def PlotVolumePerHectare(meta,mos,tv,iScn,iT,iPS,iSS,AEF):
    ...
    rat=np.maximum(0,np.minimum(1200,V_Harvest/A_Harvest))
    rat_ma=gu.movingave(rat,5,'historical')
    ax.plot(tv[iT],rat_ma[iT],'-ko',lw=1,mfc='w',ms=3,label='FCAST')
    ...
    d['V'][ind]=np.maximum(d['V'][ind],H_HBS['V All m3'])
    ...
    plt.plot(d['Year'][iT3],d['V'][iT3]/d['A'][iT3],'-bs',color=[0.27,0.49,0.79],mfc='w',ms=3,lw=1,label='FLNR')

    ax.set(position=[0.1,0.12,0.88,0.84],xlim=[tv[iT[0]]-0.5,tv[iT[-1]]+0.5],xticks=np.arange(tv[iT[0]],2250,5),xlabel='Time, years', \
           ylim=[200,1220],yticks=np.arange(0,2000,100),ylabel='Harvest volume (m$^3$ ha$^-$$^1$)')
...
def PlotGHGBalance(meta,mos,tv,AEF,iScn,iT,iPS,iSS):
    ...
    plt.close('all')
    fig,ax=plt.subplots(1,figsize=gu.cm2inch(15.5,8)); wd=1; lw=1
    ax.add_patch(Rectangle([1920,-200],100,550,fc=[0.94,0.94,0.94],ec="none"))
    ax.plot(tv[iT],np.zeros(iT.size),'k-',lw=0.25)
    ...
    ax.plot(tv[iT],mos['Scenarios'][iScn]['Sum'][v]['Ensemble Mean'][iT,iPS,iSS]/1e6*AEF,'k-',lw=lw,mfc='w',mew=0.5,ms=2,label='GHG balance (with subs.)')
    ax.plot(tv[iT],mos['Scenarios'][iScn]['Sum'][v2]['Ensemble Mean'][iT,iPS,iSS]/1e6*AEF,'k--',color=[0.45,0.45,0.45],lw=lw,mfc='w',mew=0.5,ms=2,label='GHG balance (w/o subs.)')
    
    ax.plot(tv[iT],mos['Scenarios'][iScn]['Sum']['E_CO2e_LULUCF_NEE']['Ensemble Mean'][iT,iPS,iSS]/1e6*AEF,'k-.',color=[0.4,0.6,0.15],lw=lw,mfc='w',ms=2,mew=0.5,label='Net ecosystem exchange')
    
    ax.set(position=[0.075,0.085,0.9,0.86],xlim=[tv[iT[0]]-0.5,tv[iT[-1]]+0.5],xticks=np.arange(tv[iT[0]],2250,20),ylim=[-160,160],yticks=np.arange(-200,200,20),
           ylabel='GHG fluxes (MtCO$_2$e yr$^-$$^1$)',xlabel='Time, years',aspect='auto')
    ax.legend(loc='lower right',frameon=False,facecolor='w',edgecolor='w')
    #plt.grid()
    ax.yaxis.set_ticks_position('both'); ax.xaxis.set_ticks_position('both'); ax.tick_params(length=1.5)
...
def PlotGHGBalanceAndBenefitWithCumulative(meta,mos,tv,AEF,cmp,iT,iPS,iSS):
    ...
    for iV in range(len(v1)):
        ...
        ax[0,0].plot(tv[iT],np.zeros(iT.size),lw=lw0,color=cla)
        ...
        ax[0,0].fill_between(tv[iT],ylo1,yhi1,color=cl1,alpha=Alpha,lw=0)
        ax[0,0].fill_between(tv[iT],ylo2,yhi2,color=cl2,alpha=Alpha,lw=0)
        ax[0,0].plot(tv[iT],mos['Scenarios'][iB]['Sum'][v1[iV]]['Ensemble Mean'][iT,iPS,iSS]/1e6*AEF,'-',color=cl1,lw=lw,label='Baseline')
        ax[0,0].plot(tv[iT],mos['Scenarios'][iP]['Sum'][v1[iV]]['Ensemble Mean'][iT,iPS,iSS]/1e6*AEF,'--',color=cl2,lw=lw,label='Actual')
        ...
        ax[0,1].plot(tv[iT],np.zeros(iT.size),lw=lw0,color=cla)
        ...
        cs=np.column_stack((ylo1,ylo2,yhi1,yhi2))
        ymx=np.max(np.max(cs,axis=1)); ymn=np.min(np.min(cs,axis=1))
        ax[0,1].fill_between(tv[iT],ylo1,yhi1,color=cl1,alpha=Alpha,lw=0)
        ax[0,1].fill_between(tv[iT],ylo2,yhi2,color=cl2,alpha=Alpha,lw=0)
        ax[0,1].plot(tv[iT],mos['Scenarios'][iB]['Sum'][v2[iV]]['Ensemble Mean'][iT,iPS,iSS]/1e9*AEF,'-',color=cl1,lw=lw,label='Baseline')
        ax[0,1].plot(tv[iT],mos['Scenarios'][iP]['Sum'][v2[iV]]['Ensemble Mean'][iT,iPS,iSS]/1e9*AEF,'--',color=cl2,lw=lw,label='Actual')
        for text in leg.get_texts():
            plt.setp(text, color=cla)
        ...
        ymx=np.max(np.max(cs,axis=1)); ymn=np.min(np.min(cs,axis=1))
        ax[1,0].plot(tv[iT],np.zeros(iT.size),lw=lw0,color=cla)
        ax[1,0].fill_between(tv[iT],ylo,yhi,color=cl3,alpha=Alpha,lw=0,label='95% C.I.')
        ...
        ax[1,0].fill_between(tv[iT],ylo2,yhi2,color=cl3,alpha=0.16,lw=0,label='50% C.I.')
        
        ax[1,0].plot(tv[iT],ymu,'-',lw=lw,color=cl3,label='Mean')
        ...
        cs=np.column_stack((ylo,ymu,yhi))
        ymx=np.max(np.max(cs,axis=1)); ymn=np.min(np.min(cs,axis=1))
        ax[1,1].plot(tv[iT],np.zeros(iT.size),lw=lw0,color=cla)
        ax[1,1].fill_between(tv[iT],ylo,yhi,color=cl3,alpha=Alpha,lw=0)
        ...
        ax[1,1].fill_between(tv[iT],ylo2,yhi2,color=cl3,alpha=0.16,lw=0,label='50% C.I.')
        
        ax[1,1].plot(tv[iT],ymu,'-',lw=lw,color=cl3,label='Actual minus baseline')
        ...
        gu.axletters(ax,plt,0.03,0.9,FontColor=cla,LetterStyle='Caps',FontWeight='Bold')
...
def PlotGHGBenefit(meta,mos,tv,AEF,cmp,iT,iPS,iSS):
    ...
    for iv in range(len(vL)):
        ...
        ax[0].plot(tv[iT],np.zeros(iT.size),lw=lw0,color=cla)
        ...
        cs=np.column_stack((ylo1,ylo2,yhi1,yhi2))
        ymx=np.max(np.max(cs,axis=1)); ymn=np.min(np.min(cs,axis=1))
        ax[0].fill_between(tv[iT],ylo1,yhi1,color=cl1,alpha=Alpha,lw=0)
        ax[0].fill_between(tv[iT],ylo2,yhi2,color=cl2,alpha=Alpha,lw=0)
        ax[0].plot(tv[iT],mos['Scenarios'][iB]['Sum'][v]['Ensemble Mean'][iT,iPS,iSS]/1e6*AEF,'-',color=cl1,lw=lw,label='Baseline (w/o harvesting)')
        ax[0].plot(tv[iT],mos['Scenarios'][iP]['Sum'][v]['Ensemble Mean'][iT,iPS,iSS]/1e6*AEF,'--',color=cl2,lw=lw,label='Actual (with harvesting)')
        leg=ax[0].legend(loc='upper left',bbox_to_anchor=(0.08,0.48,0.5,0.5),frameon=False,facecolor=None,edgecolor='w')
        #for text in leg.get_texts():
        #    plt.setp(text, color=cla)
        ax[0].text(tv[iT[0]]+7,0.65*np.minimum(np.abs(ymx),np.abs(ymn)),'Source',fontsize=7,style='italic',weight='bold',color=clt,va='center')
        ax[0].text(tv[iT[0]]+7,-0.65*np.minimum(np.abs(ymx),np.abs(ymn)),'Sink',fontsize=7,style='italic',weight='bold',color=clt,va='center')
        ax[0].set(position=[0.07,0.11,0.43,0.83],xlim=[tv[iT[0]],tv[iT[-1]]],xticks=np.arange(tv[iT[0]],2200,50), \
          ylim=[-160,220],yticks=np.arange(-200,300,40),ylabel='GHG balance (MtCO$_2$e yr$^-$$^1$)',xlabel='Time, years')
        ...
        cs=np.column_stack((ylo,ymu,yhi))
        ymx=np.max(np.max(cs,axis=1)); ymn=np.min(np.min(cs,axis=1))
        ax[1].plot(tv[iT],np.zeros(iT.size),lw=lw0,color=cla)
        
        ax[1].fill_between(tv[iT],ylo,yhi,color=cl3,alpha=Alpha,lw=0,label='95% C.I.')    
        ...
        ax[1].fill_between(tv[iT],ylo2,yhi2,color=cl3,alpha=0.16,lw=0,label='50% C.I.')        
        ax[1].plot(tv[iT],ymu,'-',lw=lw,color=cl3,label='Mean')        
        ax[1].set(position=[0.57,0.11,0.43,0.83],xlim=[tv[iT[0]],tv[iT[-1]]],xticks=np.arange(tv[iT[0]],2200,50),
          ylim=[ymn-0.01*np.abs(ymn),ymx+0.01*np.abs(ymx)],yticks=np.arange(-200,300,20), \
          ylabel='$\Delta$GHG (MtCO$_2$e yr$^-$$^1$)',xlabel='Time, years')
        ax[1].yaxis.set_ticks_position('both'); ax[1].xaxis.set_ticks_position('both'); ax[1].tick_params(length=tickl)
        ax[1].text(tv[iT[0]]+7,0.65*np.minimum(np.abs(ymx),np.abs(ymn)),'Net emission',fontsize=7,style='italic',weight='bold',color=clt,va='center');
        ax[1].text(tv[iT[0]]+7,-0.65*np.minimum(np.abs(ymx),np.abs(ymn)),'Net removal',fontsize=7,style='italic',weight='bold',color=clt,va='center');
        leg=ax[1].legend(loc='upper right',frameon=False,facecolor=None,edgecolor='w')
        
        gu.axletters(ax,plt,0.03,0.92,FontColor=cla,LetterStyle='Caps',FontWeight='Bold');
        nam_ps=meta['Project']['Strata']['Project']['Unique CD'][iPS];
        nam_ss=meta['Project']['Strata']['Spatial']['Unique CD'][iSS];
...
def PlotComparisonWithPIR(meta,mos,tv,AEF,iScn,iT,iPS,iSS):
    ...
    plt.close('all')
    fig,ax=plt.subplots(2,2,figsize=gu.cm2inch(16,8)); lw=0.5
    ax[0,0].plot(tv[iT],np.zeros(iT.size),'k-',lw=0.5)
    
    ax[0,0].plot(tv[iT],mos['Scenarios'][iScn]['Sum']['E_CO2e_LULUCF_NEE']['Ensemble Mean'][iT,iPS,iSS]/1e6*AEF,'ob-',lw=lw,mfc='w',mew=0.5,ms=2,label='FCAST')
    ax[0,0].plot(dPIR['Year'],dPIR['Forest Growth Minus Decay']/1e3,'rs-',lw=lw,mfc=[1,1,1],label='PIR')
    ax[0,0].set(position=[0.08,0.59,0.41,0.4],xlim=[tv[iT[0]]-0.5,tv[iT[-1]]+0.5],xticks=np.arange(tv[iT[0]],2250,20),ylim=[-100,60],yticks=np.arange(-120,300,20),
           ylabel='Growth - Decay (MtCO$_2$e yr$^-$$^1$)',xlabel='Time, years')
    ...
    ax[0,1].plot(tv[iT],mos['Scenarios'][iScn]['Sum']['E_CO2e_LULUCF_Wildfire']['Ensemble Mean'][iT,iPS,iSS]/1e6*AEF,'ob-',lw=lw,mfc='w',mew=0.5,ms=2,label='Net ecosystem exchange')
    ax[0,1].plot(dPIR['Year'],dPIR['Wildfires']/1e3,'rs-',lw=lw,mfc=[1,1,1],label='PIR')
    ax[0,1].set(position=[0.57,0.59,0.41,0.4],xlim=[tv[iT[0]]-0.5,tv[iT[-1]]+0.5],xticks=np.arange(tv[iT[0]],2250,20),ylim=[0,220],yticks=np.arange(0,300,25),
           ylabel='Wildfire (MtCO$_2$e yr$^-$$^1$)',xlabel='Time, years')
    ax[0,1].yaxis.set_ticks_position('both'); ax[0,1].xaxis.set_ticks_position('both'); ax[0,1].tick_params(length=1.5)
    
    #ax[1,0].plot(tv[iT],mos['Scenarios'][iScn]['Sum']['E_CO2e_LULUCF_OpenBurning']['Ensemble Mean'][iT,iPS,iSS]/1e6*AEF,'ob-',lw=lw,mfc='w',mew=0.5,ms=2,label='Net ecosystem exchange')
    #ax[1,0].plot(dPIR['Year'],dPIR['Slash Pile Burning']/1e3,'rs-',lw=lw,mfc=[1,1,1],label='PIR')
    ax[1,0].plot(tv[iT],mos['Scenarios'][iScn]['Sum']['E_CO2e_LULUCF_HWP']['Ensemble Mean'][iT,iPS,iSS]/1e6*AEF,'ob-',lw=lw,mfc='w',mew=0.5,ms=2,label='HWP decay (FCAST)')
    ax[1,0].plot(tv[iT],(mos['Scenarios'][iScn]['Sum']['E_CO2e_LULUCF_HWP']['Ensemble Mean'][iT,iPS,iSS]+mos['Scenarios'][iScn]['Sum']['E_CO2e_ESC_Bioenergy']['Ensemble Mean'][iT,iPS,iSS])/1e6*AEF,'oc-',lw=lw,mfc='w',mew=0.5,ms=2,label='HWP decay + bioenergy (FCAST)')
    #ax[1,0].plot(tv[iT],gu.movingave(mos['Scenarios'][iScn]['Sum']['C_ToMill']['Ensemble Mean'][iT,iPS,iSS]/1e6*AEF*3.667,10,'center'),'og-',lw=lw,mfc='w',mew=0.5,ms=2,label='Harvest removals (FCAST)')
    ax[1,0].plot(dPIR['Year'],dPIR['Decomposition of Harvested Wood Products']/1e3,'rs-',lw=lw,mfc=[1,1,1],label='HWP decay (PIR)')
    ax[1,0].set(position=[0.08,0.09,0.41,0.4],xlim=[tv[iT[0]]-0.5,tv[iT[-1]]+0.5],xticks=np.arange(tv[iT[0]],2250,20),ylim=[0,100],yticks=np.arange(-120,300,20),
           ylabel='HWP (MtCO$_2$e yr$^-$$^1$)',xlabel='Time, years')
    ...
    ax[1,1].plot(tv[iT],mos['Scenarios'][iScn]['Sum']['E_CO2e_AGHGB_WSub']['Ensemble Mean'][iT,iPS,iSS]/1e6*AEF,'ob-',lw=lw,mfc='w',mew=0.5,ms=2,label='With subs. (FCAST)')
    ax[1,1].plot(tv[iT],mos['Scenarios'][iScn]['Sum']['E_CO2e_AGHGB_WOSub']['Ensemble Mean'][iT,iPS,iSS]/1e6*AEF,'sb-',color='c',mec='c',lw=lw,mfc='w',mew=0.5,ms=2,label='W/O subs. (FCAST)')
    ax[1,1].plot(dPIR['Year'],dPIR['Forest Management']/1e3,'rs-',lw=0.25,mfc='w',label='Forest Management (PIR)')
    ax[1,1].set(position=[0.57,0.09,0.41,0.4],xlim=[tv[iT[0]]-0.5,tv[iT[-1]]+0.5],xticks=np.arange(tv[iT[0]],2250,20),ylim=[-150,250],yticks=np.arange(-150,300,50),
           ylabel='GHG balance (MtCO$_2$e yr$^-$$^1$)',xlabel='Time, years')
    ...
    #plt.grid()
    gu.axletters(ax,plt,0.03,0.885,FontColor=cla,LetterStyle='Caps',FontWeight='Bold')
    if flag_printfigs=='On':
        nam_ps=meta['Project']['Strata']['Project']['Unique CD'][iPS]
        nam_ss=meta['Project']['Strata']['Spatial']['Unique CD'][iSS]
...
def PlotAreaDisturbed(meta,mos,tv,AEF,ivlT,iScn,iT,iPS,iSS):
    ...
    ax[0].set(position=[0.06,0.56,0.92,0.43],yscale='linear', \
      xlim=[yr_start,np.max(A['tv'])],xticks=np.arange(np.min(A['tv']),np.max(A['tv'])+1,20),ylabel='Area affected (ha x 1000)');
    ...
    ax[1].set(position=[0.06,0.08,0.92,0.43],yscale='linear',
      xlim=[yr_start,np.max(A['tv'])],xticks=np.arange(np.min(A['tv']),np.max(A['tv'])+1,20),xlabel='Time, years',ylabel='Area affected (ha x 1000)');
    ax[1].tick_params(length=1.5)
...
def PlotCarbonFluxTS(meta,mos,tv,iT,iB,iP,iPS,iSS):
    ...
    ax[0,0].fill_between(tv[iT],lo,hi,color=cl2,alpha=Alpha,lw=0)
    ax[0,0].plot(tv[iT],mu,'-',color=cl2,label='Actual scenario',lw=lw+0.25)    
    ...
    ax[0,0].fill_between(tv[iT],lo,hi,color=cl1,alpha=Alpha,lw=0)
    ax[0,0].plot(tv[iT],mu,'--',color=cl1,label='Baseline scenario',lw=lw+0.25)       
    ax[0,0].set(position=[0.07,0.57,0.42,0.42],xlim=[tv[iT[0]]-0.5,tv[iT[-1]]+0.5],xticks=np.arange(0,2220,25), \
      xlabel='Time, years',ylabel='NEE (tCO$_2$e ha$^-$$^1$yr$^-$$^1$)')
    ...
    ax[0,1].fill_between(tv[iT],lo,hi,color=cl2,alpha=Alpha,lw=0)
    ax[0,1].plot(tv[iT],mu,'-',color=cl2,label='Actual scenario',lw=lw+0.25)
    ...
    ax[0,1].fill_between(tv[iT],lo,hi,color=cl1,alpha=Alpha,lw=0)
    ax[0,1].plot(tv[iT],mu,'--',color=cl1,label='Baseline scenario',lw=lw+0.25)       
    ax[0,1].set(position=[0.57,0.57,0.42,0.42],yscale='linear',xlim=[tv[iT[0]]-0.5,tv[iT[-1]]+0.5], \
      xticks=np.arange(0,2220,25),xlabel='Time, years',ylabel='Wildfire emissions (tCO$_2$e ha$^-$$^1$yr$^-$$^1$)')
    ax[0,1].yaxis.set_ticks_position('both'); ax[0,1].xaxis.set_ticks_position('both'); ax[0,1].tick_params(length=1.5)
    ...
    ax[1,0].fill_between(tv[iT],lo,hi,color=cl2,alpha=Alpha,lw=0)
    ax[1,0].plot(tv[iT],mu,'-',color=cl2,label='Actual scenario',lw=lw+0.25)    
    ...
    ax[1,0].fill_between(tv[iT],lo,hi,color=cl1,alpha=Alpha,lw=0)
    ax[1,0].plot(tv[iT],mu,'--',color=cl1,label='Baseline scenario',lw=lw+0.25)    
    ax[1,0].set(position=[0.07,0.07,0.42,0.42],xlim=[tv[iT[0]]-0.5,tv[iT[-1]]+0.5],xticks=np.arange(0,2220,25), \
      xlabel='Time, years',ylabel='Open burning emissions (tCO$_2$e ha$^-$$^1$yr$^-$$^1$)')
    ax[1,0].yaxis.set_ticks_position('both'); ax[1,0].xaxis.set_ticks_position('both'); ax[1,0].tick_params(length=1.5)
    ...
    ax[1,1].fill_between(tv[iT],lo,hi,color=cl2,alpha=Alpha,lw=0)
    ax[1,1].plot(tv[iT],mu,'-',color=cl2,label='Actual scenario',lw=lw+0.25)
    ...
    ax[1,1].fill_between(tv[iT],lo,hi,color=cl1,alpha=Alpha,lw=0)
    ax[1,1].plot(tv[iT],mu,'--',color=cl1,label='Baseline scenario',lw=lw+0.25)       
    ax[1,1].set(position=[0.57,0.07,0.42,0.42],xlim=[tv[iT[0]]-0.5,tv[iT[-1]]+0.5],xticks=np.arange(0,2220,25), \
      xlabel='Time, years',ylabel='Product emissions (tCO$_2$e ha$^-$$^1$yr$^-$$^1$)')
    ax[1,1].yaxis.set_ticks_position('both'); ax[1,1].xaxis.set_ticks_position('both'); ax[1,1].tick_params(length=1.5)
...
def PlotCarbonPoolTS(meta,mos,tv,iT,iB,iP,iPS,iSS):
    ...
    ax[0,0].fill_between(tv[iT],lo,hi,color=cl1,alpha=Alpha,lw=0)
    ax[0,0].plot(tv[iT],mu,'-',color=cl1,label='Actual scenario',lw=lw+0.25)    
    ...
    ax[0,0].fill_between(tv[iT],lo,hi,color=cl0,alpha=Alpha,lw=0)
    ax[0,0].plot(tv[iT],mu,'--',color=cl0,label='Baseline scenario',lw=lw+0.25)       
    ax[0,0].set(position=[0.07,0.57,0.42,0.42],xlim=[tv[iT[0]]-0.5,tv[iT[-1]]+0.5],xticks=np.arange(0,2220,25), \
      xlabel='Time, years',ylabel='Biomass (MgC ha$^-$$^1$)')
    ...
    mu=mos['Scenarios'][iP]['Mean'][v]['Ensemble Mean'][iT,iPS,iSS]
    ax[0,1].fill_between(tv[iT],lo,hi,color=cl1,alpha=Alpha,lw=0)
    ax[0,1].plot(tv[iT],mu,'-',color=cl1,label='Actual scenario',lw=lw+0.25)    
    lo=mos['Scenarios'][iB]['Mean'][v]['Ensemble P025'][iT,iPS,iSS]
    hi=mos['Scenarios'][iB]['Mean'][v]['Ensemble P975'][iT,iPS,iSS]
    mu=mos['Scenarios'][iB]['Mean'][v]['Ensemble Mean'][iT,iPS,iSS]
    ax[0,1].fill_between(tv[iT],lo,hi,color=cl0,alpha=Alpha,lw=0)
    ax[0,1].plot(tv[iT],mu,'--',color=cl0,label='Baseline scenario',lw=lw+0.25)       
    ax[0,1].set(position=[0.57,0.57,0.42,0.42],yscale='linear',xlim=[tv[iT[0]]-0.5,tv[iT[-1]]+0.5],xticks=np.arange(0,2220,25), \
      xlabel='Time, years',ylabel='Dead wood (MgC ha$^-$$^1$)')
    ...
    mu=mos['Scenarios'][iP]['Mean'][v]['Ensemble Mean'][iT,iPS,iSS]
    ax[1,0].fill_between(tv[iT],lo,hi,color=cl1,alpha=Alpha,lw=0)
    ax[1,0].plot(tv[iT],mu,'-',color=cl1,label='Actual scenario',lw=lw+0.25)    
    lo=mos['Scenarios'][iB]['Mean'][v]['Ensemble P025'][iT,iPS,iSS]
    hi=mos['Scenarios'][iB]['Mean'][v]['Ensemble P975'][iT,iPS,iSS]
    mu=mos['Scenarios'][iB]['Mean'][v]['Ensemble Mean'][iT,iPS,iSS]
    ax[1,0].fill_between(tv[iT],lo,hi,color=cl0,alpha=Alpha,lw=0)
    ax[1,0].plot(tv[iT],mu,'--',color=cl0,label='Baseline scenario',lw=lw+0.25)    
    ax[1,0].set(position=[0.07,0.07,0.42,0.42],xlim=[tv[iT[0]]-0.5,tv[iT[-1]]+0.5],xticks=np.arange(0,2220,25), \
      xlabel='Time, years',ylabel='Litter (MgC ha$^-$$^1$)')
    ...
    mu=mos['Scenarios'][iP]['Mean'][v]['Ensemble Mean'][iT,iPS,iSS]
    ax[1,1].fill_between(tv[iT],lo,hi,color=cl1,alpha=Alpha,lw=0)
    ax[1,1].plot(tv[iT],mu,'-',color=cl1,label='Actual scenario',lw=lw+0.25)
    lo=mos['Scenarios'][iB]['Mean'][v]['Ensemble P025'][iT,iPS,iSS]
    hi=mos['Scenarios'][iB]['Mean'][v]['Ensemble P975'][iT,iPS,iSS]
    mu=mos['Scenarios'][iB]['Mean'][v]['Ensemble Mean'][iT,iPS,iSS]
    ax[1,1].fill_between(tv[iT],lo,hi,color=cl0,alpha=Alpha,lw=0)
    ax[1,1].plot(tv[iT],mu,'--',color=cl0,label='Baseline scenario',lw=lw+0.25)       
    ax[1,1].set(position=[0.57,0.07,0.42,0.42],xlim=[tv[iT[0]]-0.5,tv[iT[-1]]+0.5],xticks=np.arange(0,2220,25), \
      xlabel='Time, years',ylabel='Soil (MgC ha$^-$$^1$)')
...
def PlotNetGrowthTS(meta,mos,tv,C_M_ByAgent,iT,iScn,iPS,iSS):
    ...
    ax.fill_between(tv[iT],lo,hi,color=cl1,alpha=Alpha,lw=0)
    ax.plot(tv[iT],mu,'-',color=cl1,label='Net growth (w/o instects)',lw=lw+0.25)
    ...
    #ax.fill_between(tv[iT],lo1,hi1,color='r',alpha=Alpha,lw=0)
    #ax.plot(tv[iT],mu1,'-',color='r',label='Mortality',lw=lw+0.25)
    ...
    #ax.fill_between(tv[iT],lo3,hi3,color='y',alpha=Alpha,lw=0)
    #ax.plot(tv[iT],mu3,'-',color='y',label='Mortality',lw=lw+0.25)    
    
    mth='center'
    lo4=gu.movingave(lo-Mb,7,mth) 
    hi4=gu.movingave(hi-Mb,7,mth) 
    mu4=gu.movingave(mu-Mb,7,mth)    
    ax.fill_between(tv[iT],lo4,hi4,color='y',alpha=Alpha,lw=0)
    ax.plot(tv[iT],mu4,'-',color='y',label='Net growth (with insects)',lw=lw+0.25)    
    ...
    ax.set(position=[0.07,0.07,0.84,0.84],xlim=[tv[iT[0]]-0.5,tv[iT[-1]]+0.5],
