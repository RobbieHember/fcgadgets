...
def Eval_BiomassDynamicsAve_CN(meta,pNam,gpt):
    ...
    for i in range(u.size):
        lab[i]=ugp.lut_id2cd(meta['LUT']['GP']['Ecozone BC L1'],u[i])
        for v in gpt['vaL']:
            ind=np.where( (gpt['Ecozone BC L1']==u[i]) & (gpt['PTF CN']==1) & (gpt['Ctot Net']>=-1000) & (gpt['Ctot Net']<1000) )[0]
            d[v]['N'][i]=ind.size
    ...
    ax.plot([0,5],[0,0],'k-',color=meta['Graphics']['gp']['cla'],lw=meta['Graphics']['gp']['lw1'])
    ax.bar(1-barw/2-0.01,wa['Ctot G Tot']['mu'],barw,facecolor=cl[0,:],label='Ground plot observations')
    ax.bar(1+barw/2+0.01,wa['Mod C_G_Gross_Tot']['mu'],barw,facecolor=cl[1,:],label='Predictions (FCS)')
    ...
    ax.bar(2-barw/2-0.01,-wa['Ctot Mort Nat']['mu'],barw,facecolor=cl[0,:])
    ax.bar(2+barw/2+0.01,-wa['Mod C_M_Nat']['mu'],barw,facecolor=cl[1,:])
    ...
    ax.bar(3-barw/2-0.01,-wa['Ctot Mort Harv']['mu'],barw,facecolor=cl[0,:])
    ax.bar(3+barw/2+0.01,-wa['Mod C_M_Harv']['mu'],barw,facecolor=cl[1,:])
    ...
    ax.bar(4-barw/2-0.01,wa['Ctot Net']['mu'],barw,facecolor=cl[0,:])
    ax.bar(4+barw/2+0.01,wa['Mod C_G_Net']['mu'],barw,facecolor=cl[1,:])
...
def EvalGrossGrowthByBGC_CN(meta,gplt):
    ...
    ax.set(position=[0.1,0.1,0.86,0.86],xlabel='Observed gross growth (tC ha$^{-1}$ yr$^{-1}$)',ylabel='Predicted gross growth (tC ha$^{-1}$ yr$^{-1}$)',xlim=[0,3.6],ylim=[0,3.6])
    #plt.legend(frameon=False,facecolor=[1,1,1],labelspacing=0.25)
    ax.yaxis.set_ticks_position('both'); ax.xaxis.set_ticks_position('both'); ax.tick_params(length=meta['Graphics']['gp']['tickl'])
    if meta['Graphics']['Print Figures']=='On':
    ...
    #for i in range(u.size):
    #    ax.text(i,8,str(d['Csw L t0']['N'][i].astype(int)),color='k',ha='center',fontsize=8)
    ax.set(position=[0.08,0.12,0.9,0.86],xticks=np.arange(u.size),xticklabels=lab,ylabel='Gross growth (tC ha$^{-1}$ yr$^{-1}$)',xlim=[-0.5,u.size-0.5],ylim=[0,3.5])
    plt.legend(frameon=False,facecolor=[1,1,1],labelspacing=0.25)
    ax.yaxis.set_ticks_position('both'); ax.xaxis.set_ticks_position('both'); ax.tick_params(length=meta['Graphics']['gp']['tickl'])
    if meta['Graphics']['Print Figures']=='On':
...
def EvalSOCByBGC_ShawComp(meta,pNam,gplt):
    ...
    ax.bar(np.arange(u.size)-barw/2-0.01,d['ORG_C_THA']['mu'],barw,facecolor=cl[2,:],bottom=d['MIN_C_THA']['mu'],label='Ground plot observations, organic horizon (Shaw et al. 2018)')
    ax.bar(np.arange(u.size)+barw/2+0.01,d['C_Soil_Tot']['mu'],barw,facecolor=cl[1,:],label='Prediction, mineral horizon (FCS)')
...
def EvalAgeResponsesBiomassAndNetGrowth_ByReg(meta,pNam,gpt):
    ...
    lw=0.5; ms=3; cl=meta['Graphics']['GP Comp']['cl']
    ...
    ax1[0].plot(bin,mu,'-ko',ms=ms,lw=lw,mew=lw,color=cl[0,:],mfc='w',mec=cl[0,:],label='Observed biomass',zorder=1)
    N,mu,med,sig,se=gu.discres(xM,yM,bw,bin)
    ax1[0].plot(bin,mu,'--ks',ms=ms,lw=lw,mew=lw,color=cl[1,:],mfc='w',mec=cl[1,:],label='Predicted biomass',zorder=1)
    ...
    N,mu,med,sig,se=gu.discres(xO,yO,bw,bin)
    ax2.bar(bin-(0.45*bw/2),mu,0.4*bw,ec='none',fc=cl[0,:],zorder=-1)
    N,mu,med,sig,se=gu.discres(xM,yM,bw,bin)
    ax2.bar(bin+(0.45*bw/2),mu,0.45*bw,ec='none',fc=cl[1,:],zorder=-1)
    ...
    yM=gpt['Mod C_Biomass_Tot t0'][ind]
    N,mu,med,sig,se=gu.discres(xO,yO,bw,bin)
    ax1[1].plot(bin,mu,'-ko',ms=ms,lw=lw,mew=lw,color=cl[0,:],mfc='w',mec=cl[0,:],zorder=1)
    N,mu,med,sig,se=gu.discres(xM,yM,bw,bin)
    ax1[1].plot(bin,mu,'--ks',ms=ms,lw=lw,mew=lw,color=cl[1,:],mfc='w',mec=cl[1,:],zorder=1)
    ...
    yM=gpt['Mod C_G_Net'][ind]
    N,mu,med,sig,se=gu.discres(xO,yO,bw,bin)
    ax3.bar(bin-(0.45*bw/2),mu,0.4*bw,ec='none',fc=cl[0,:],label='Observed net growth',zorder=-1)
    N,mu,med,sig,se=gu.discres(xM,yM,bw,bin)
    ax3.bar(bin+(0.45*bw/2),mu,0.45*bw,ec='none',fc=cl[1,:],label='Predicted net growth',zorder=-1)
...
def EvalAgeResponsesGrossGrowthAndMortality_ByReg(meta,pNam,gpt):
    ...
    N,mu,med,sig,se=gu.discres(xO,yO,bw,bin)
    ax1[0].bar(bin-(0.45*bw/2),mu,0.4*bw,ec='none',fc=cl[0,:],label='Observed gross growth')
    ax1[0].errorbar(bin-(0.45*bw/2),mu,yerr=se,color=0.5*cl[0,:],fmt='none',capsize=1.5,lw=0.25,markeredgewidth=0.5)
    N,mu,med,sig,se=gu.discres(xM,yM,bw,bin)
    ax1[0].bar(bin+(0.45*bw/2),mu,0.4*bw,ec='none',fc=cl[1,:],label='Predicted gross growth')
    ax1[0].errorbar(bin+(0.45*bw/2),mu,yerr=se,color=0.5*cl[1,:],fmt='none',capsize=1.5,lw=0.25,markeredgewidth=0.5)
    ...
    N,mu,med,sig,se=gu.discres(xO,yO,bw,bin)
    ax1[0].bar(bin-(0.45*bw/2),-mu,0.4*bw,ec='none',fc=cl[4,:],label='Observed mortality')
    ax1[0].errorbar(bin-(0.45*bw/2),-mu,yerr=se,color=0.5*cl[4,:],fmt='none',capsize=1.5,lw=0.25,markeredgewidth=0.5)
    N,mu,med,sig,se=gu.discres(xM,yM,bw,bin)
    ax1[0].bar(bin+(0.45*bw/2),-mu,0.4*bw,ec='none',fc=cl[5,:],label='Predicted mortality')
    ax1[0].errorbar(bin+(0.45*bw/2),-mu,yerr=se,color=0.5*cl[5,:],fmt='none',capsize=1.5,lw=0.25,markeredgewidth=0.5)
    ax1[0].plot([0,300],[0,0],'k-')
    ...
    ax1[0].yaxis.set_ticks_position('both'); ax1[0].xaxis.set_ticks_position('both'); ax1[0].tick_params(length=meta['Graphics']['gp']['tickl'])
    ax1[0].legend(loc='lower left',frameon=False,facecolor=[1,1,1],labelspacing=0.25)
    # Interior
    ind=np.where( (gpt['PTF CN']==1) & (gpt['Ecozone BC L1']!=meta['LUT']['GP']['Ecozone BC L1']['CWH']) & (gpt['Ecozone BC L1']!=meta['LUT']['GP']['Ecozone BC L1']['MH']) )[0]
    xO=gpt['Age Med t0'][ind]
    xM=gpt['Mod A t0'][ind]
    ...
    yM=gpt['Mod C_G_Gross_Tot'][ind]
    N,mu,med,sig,se=gu.discres(xO,yO,bw,bin)
    ax1[1].bar(bin-(0.45*bw/2),mu,0.4*bw,ec='none',fc=cl[0,:])
    ax1[1].errorbar(bin-(0.45*bw/2),mu,yerr=se,color=0.5*cl[0,:],fmt='none',capsize=1.5,lw=0.25,markeredgewidth=0.5)
    N,mu,med,sig,se=gu.discres(xM,yM,bw,bin)
    ax1[1].bar(bin+(0.45*bw/2),mu,0.4*bw,ec='none',fc=cl[1,:])
    ax1[1].errorbar(bin+(0.45*bw/2),mu,yerr=se,color=0.5*cl[1,:],fmt='none',capsize=1.5,lw=0.25,markeredgewidth=0.5)
    ...
    yM=gpt['Mod C_M_Tot'][ind]
    N,mu,med,sig,se=gu.discres(xO,yO,bw,bin)
    ax1[1].bar(bin-(0.45*bw/2),-mu,0.4*bw,ec='none',fc=cl[4,:])
    ax1[1].errorbar(bin-(0.45*bw/2),-mu,yerr=se,color=0.5*cl[4,:],fmt='none',capsize=1.5,lw=0.25,markeredgewidth=0.5)
    N,mu,med,sig,se=gu.discres(xM,yM,bw,bin)
    ax1[1].bar(bin+(0.45*bw/2),-mu,0.4*bw,ec='none',fc=cl[5,:])
    ax1[1].errorbar(bin+(0.45*bw/2),-mu,yerr=se,color=0.5*cl[5,:],fmt='none',capsize=1.5,lw=0.25,markeredgewidth=0.5)
    ax1[1].plot([0,300],[0,0],'k-')    
    ...
    plt.tight_layout()
    if meta['Graphics']['Print Figures']=='On':
        gu.PrintFig(meta['Graphics']['Print Figure Path'] + '\\QA_AgeResponseGrossGrowthAndMortality','png',900)
    return
...
def Prepare_ObsVsModComparison(meta,pNam):
    ...
    metaGP,gplt=ugp.ImportGroundPlotData(meta,type='Stand')
        ...
        for iBat in range(meta[pNam]['Project']['N Batch']):
            ...
            for v in vmL:
                if v=='C_M_Harvest_Salvage':
                    ...
                    dM[v][:,indBat]=dM[v][:,indBat]+d1[v].copy()
                    d1[v]=np.zeros( (tvSaved.size,indBat.size),dtype='float32')
                    idx=d1['C_M_ByAgent']['Harvest']['idx']                    
                    d1[v][idx[0],idx[1]]=meta['Core']['Scale Factor C_M_ByAgent']*d1['C_M_ByAgent']['Harvest']['M'].astype('float32')
                    dM[v][:,indBat]=dM[v][:,indBat]+d1[v].copy()
                else:
    ...
    for i in range(meta['Geos']['Sparse']['X'].size):
        ...
        for j in range(iS.size):
            if np.isnan(gplt['Year t1'][iS[j]])==True:
                ...
                gplt['Mod C_M_Harv'][iS[j]]=np.mean(dM['C_M_Harv'][iT,i])
                gplt['Mod C_M_Nat'][iS[j]]=np.mean(dM['C_M_Nat'][iT,i])
    ...
    gplt['vaL']=['Age VRI t0','Cbk L t0','Cbr L t0','Cf L t0','Csw L t0','Cr L t0','Cag L t0','Ctot L t0','Cdw t0',
