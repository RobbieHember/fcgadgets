...
#%% Import metaTile['Paths']

metaTile=gu.ipickle(r'D:\Data\FCI_Projects\TilesBC\MetaTiles.pkl')
...
missing_geo_atu_list=gu.ipickle(metaTile['Paths']['Results'] + '\\missing_geo_atu_list.pkl')
missing_geo_op_geos=gu.ipickle(metaTile['Paths']['Results'] + '\\missing_geo_op_geos.pkl')
missing_geo_fc_geos=gu.ipickle(metaTile['Paths']['Results'] + '\\missing_geo_fc_geos.pkl')
...
dMisFC=gu.ipickle(metaTile['Paths']['Results'] + '\\missing_geo_fc_list.pkl')
...
for hTile in range(TilesToRun_ij.shape[0]):
    ...
    geos=tu.GetTileInfo(metaTile,iTile,jTile)
    geos['X']=np.tile(np.reshape(geos['X'],(1,-1)),(geos['m'],1))
    geos['Y']=np.tile(np.reshape(geos['Y'],(-1,1)),(1,geos['n']))
    geos.keys()

    # Transform
    trf=from_origin(geos['xmin'],geos['ymax'],geos['Cellsize'],geos['Cellsize'])
    ...
    for iLyr in range(len(InvLyrInfo)):
            ...
            for feat in source:
                ...
                for i in range(len(coords0)):
                    ...
                    for j in range(len(coords1)):
                        ...
                        if np.max(x_feat)<geos['xmin']:
                            flg_outside=1
                        if np.min(x_feat)>geos['xmax']:
                            flg_outside=1
                        if np.max(y_feat)<geos['ymin']:
                            flg_outside=1
                        if np.min(y_feat)>geos['ymax']:
                            flg_outside=1
                ...
                z0=np.zeros(geos['X'].shape,dtype=float)
        ...
        # Save
        gu.opickle(geos['PathTileGeospatial'] + '\\' + InvLyrInfo[iLyr]['Layer Name'] + '.pkl',data)
        gu.opickle(geos['PathTileGeospatial'] + '\\' + InvLyrInfo[iLyr]['Layer Name'] + '_IdxToInv.pkl',IdxToInv)

        # Time
        print(str(np.round((time.time()-t0)/60)) + ' min')
    ...
    #pl=gu.ipickle(metaTile['Paths']['TileGeospatial'] + '\\RSLT_PLANTING_SVW.pkl')
    ...
    # Open AT sparse grid
    atu=gu.ipickle(geos['PathTileGeospatial'] + '\\RSLT_ACTIVITY_TREATMENT_SVW.pkl')

    #lut_atu=gu.ipickle(metaTile['Paths']['Results'] + '\\LUTs_RSLT_ACTIVITY_TREATMENT_SVW.pkl')
    ...
    # Save
    gu.opickle(geos['PathTileGeospatial'] + '\\RSLT_PLANTING_SVW.pkl',data)
    gu.opickle(geos['PathTileGeospatial'] + '\\RSLT_PLANTING_SVW_IdxToInv.pkl',IdxToInv)
    ...
    if flg==1:
        ...
        #meta['metaTile['Paths']']['Model Code']=r'C:\Users\rhember\Documents\Code_Python\fcgadgets\cbrunner'
        fcinv=gu.ipickle(geos['PathTileGeospatial'] + '\\RSLT_FOREST_COVER_INV_SVW.pkl')
        fcsilv=gu.ipickle(geos['PathTileGeospatial'] + '\\RSLT_FOREST_COVER_SILV_SVW.pkl')

        # Save old versions as alterntive names in case you need to redo this/troubleshoot a problem
        flg=1
        if flg==1:
            gu.opickle(geos['PathTileGeospatial'] + '\\RSLT_FOREST_COVER_INV_SVW_before_adding_archive.pkl',fcinv)
            gu.opickle(geos['PathTileGeospatial'] + '\\RSLT_FOREST_COVER_SILV_SVW_before_adding_archive.pkl',fcsilv)

        # Load initial layers
        flg=0
        if flg==1:
            fcinv=gu.ipickle(geos['PathTileGeospatial'] + '\\RSLT_FOREST_COVER_INV_SVW_before_adding_archive.pkl')
            fcsilv=gu.ipickle(geos['PathTileGeospatial'] + '\\RSLT_FOREST_COVER_SILV_SVW_before_adding_archive.pkl')

        # Run script
        # *** requires meta - just do a quick and dirty mock up of it and required content to run AddArchive script ***
        meta={}
        metaTile['Paths']=metaTile['Paths']
        metaTile['Paths']['Model Code']=r'C:\Users\rhember\Documents\Code_Python\fcgadgets\cbrunner'
        ...
        # Save
        gu.opickle(geos['PathTileGeospatial'] + '\\RSLT_FOREST_COVER_INV_SVW.pkl',fcinv)
        gu.opickle(geos['PathTileGeospatial'] + '\\RSLT_FOREST_COVER_SILV_SVW.pkl',fcsilv)

# Total time
print(str(np.round((time.time()-t_start)/60)) + ' min')