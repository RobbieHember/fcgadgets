...
def GetTileInfo(Paths,iTile,jTile):
    ...
    info['ix']=np.where((zTSA.X[0,:]>=np.min(info['xv'])) & (zTSA.X[0,:]<=np.max(info['xv'])))[0]
    info['iy']=np.where((zTSA.Y[:,0]>=np.min(info['yv'])) & (zTSA.Y[:,0]<=np.max(info['yv'])))[0]
    
    ind=np.ix_(info['ix'],info['iy'])
    info['X']=zTSA.X[ind]
    info['Y']=zTSA.Y[ind]
...
def GetIntersectingTilesAndMajorWatersheds():
    ...
    wat=gpd.read_file(r'Z:\!Workgrp\Forest Carbon\Data\Basemaps\Basemaps.gdb',layer='BC_MAJOR_WATERSHEDS')    
